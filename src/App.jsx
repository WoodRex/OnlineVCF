...
export default function App() {
  ...
  useEffect(() => {
    if (!dataFetchedRef.current) {
      fetch("https://s98bqp.sse.codesandbox.io/")
        .then((data) => data.json())
        ...
        {apiInfo && (
          <div className="stock-container" id="stock-container">
            ...
            <div className="stock-price">
              <span id="buy">
                {ticket && "Buy:"} {ticket?.buyPrice}
              </span>
              <br />
              <span id="sold">
                {ticket && "Sold:"} {ticket?.soldPrice}
              </span>
              <br />
              <span id="cut">
                {ticket && "Cut:"} {ticket?.cutOff}
              </span>
            </div>
            ...
            <div className="button-container">
              <button
                ...
                className="button"
                onClick={() =>
                  setTicket(apiInfo[Math.floor(Math.random() * apiInfo.length)])
                }
              >
                New Stock
              </button>
